...
import { apiInterceptors, newDialogue, getAppList } from '@/client/api';
...
import { Button, Input, Space, Typography, Badge, Avatar, Upload, Popover, message, Dropdown, MenuProps } from 'antd';
    ...
    <div className="relative group w-10 h-10 rounded-lg border border-gray-100 dark:border-gray-700 overflow-hidden flex-shrink-0 bg-white dark:bg-[#1F1F1F]">
      {file.type.startsWith('image/') ? (
        ...
        <div className="w-full h-full flex flex-col items-center justify-center bg-gray-50 dark:bg-gray-800 gap-1">
           ...
           <span className="text-[10px] text-gray-400 truncate w-full text-center px-1">{file.name}</span>
        </div>
      )}
      <div 
...
export default function Home() {
    ...
    },
    {
      onSuccess: (data) => {
        if (data?.app_list) {
          ...
          const defaultApp = data.app_list.find(app => app.app_code === 'chat_normal') || data.app_list[0];
  ...
  const onSubmit = async () => {
    ...
    if (res) {
      localStorage.setItem(STORAGE_INIT_MESSAGE_KET, JSON.stringify({ 
        id: res.conv_uid, 
        message: userInput,
    ...
    items: appList.map(app => ({
      ...
      label: (
        <div className="flex items-center gap-2" onClick={() => setSelectedApp(app)}>
          <span className="text-base">{app.icon ? <img src={app.icon} className="w-4 h-4" /> : '🤖'}</span>
  ...
  return (
    <div className='h-screen flex flex-col bg-[#FAFAFA] dark:bg-[#111] overflow-y-auto relative'>
      ...
      <div className='flex-1 flex flex-col items-center justify-center w-full max-w-5xl mx-auto px-4 mt-10'>
        ...
        <div 
          className={cls(
            ...
            isFocus ? "border-blue-500/50 shadow-lg ring-4 ring-blue-500/5" : "border-gray-200 dark:border-gray-800"
          )}
          onDragOver={(e) => {
            e.preventDefault();
            setIsFocus(true);
          ...
          <div className="p-4">
            ...
            <div className="flex items-center justify-between px-2 pb-1">
              <div className="flex items-center gap-2">
                <Popover content={plusMenuContent} trigger="click" placement="topLeft" overlayClassName="!p-0">
                  <Button 
                    shape="circle" 
                    icon={<PlusOutlined />} 
                 ...
                 <Dropdown menu={appMenuProps} trigger={['click']} placement="bottomLeft">
                  <div className="flex items-center gap-2 bg-gray-50 dark:bg-gray-800/50 px-3 py-1.5 rounded-full border border-gray-100 dark:border-gray-700/50 cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors">
                    <span className="text-base">{selectedApp?.icon ? <img src={selectedApp.icon} className="w-4 h-4" /> : '🤖'}</span>
                    <span className="text-sm text-gray-700 dark:text-gray-300 font-medium max-w-[100px] truncate">{selectedApp?.app_name || t('select_app', 'Select App')}</span>
