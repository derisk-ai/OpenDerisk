...
**并发约束**：虽然支持并行调用，但必须确保每个启动的 Agent 任务目标是**完全独立且互斥**的。
1. **禁止冗余**：严禁同时启动多个 Agent 执行相似、重复或包含关系的任务。
2. **禁止重复**：启动前必须检查历史记录，严禁重复执行已经成功或正在执行的任务目标。
    ...
    async def run(
        ...
        **kwargs,
        ...
        try:
            ...
            logger.warning(
                f"[AgentAction] Looking for agent with agent_name={action_input.agent_name}"
            )
            recipient = next(
                (
                    agent
                    for agent in sender.agents
                    if agent.name == action_input.agent_name
                    or agent.agent_context.agent_app_code == action_input.agent_name
                ),
                None,
            )
            if not recipient:
            ...
            message = AgentMessage.init_new(
                ...
                context=(received_message.context or {})
                | (action_input.extra_info or {}),
                rounds=await sender.memory.gpts_memory.next_message_rounds(
            ...
            answer: AgentMessage = await sender.send(
                message=message,
                recipient=recipient,
                request_reply=True,
            ...
            if (
                isinstance(recipient, ScheduledAgent)
                and recipient.scheduler
                and recipient.scheduler.running()
            ):
            ...
            ask_user = (
                True
                if answer
                and answer.action_report
                and any([act_out.ask_user for act_out in answer.action_report])
                else False
            )
            ## 终止状态要排除正常返回的报告Agent
            ...
            logger.info(
                f"[ACTION]---------->   Agent Action [{sender.name}] --> answer: {answer}"
            )
        ...
        **kwargs,
        ...
        if tool_call.name == cls.name:
            ...
            if tool_call.args.get("background"):
                extra_info: Dict = {"background": tool_call.args.get("background")}

            return cls(
                action_uid=tool_call.tool_call_id,
                action_input=AgentActionInput(
                    agent_name=tool_call.args.get("agent_id"),
