...
class GptsMemory:
    ...
    async def push_message(
        ...
        if gpt_msg and gpt_msg.sender == UserProxyAgent().role:
            return
        final_view = await self.vis_messages(conv_id, gpt_msg, stream_msg, is_first_chunk=is_first_chunk,
    ...
    async def append_message(self, conv_id: str, message: GptsMessage, incremental: bool = False, save_db: bool = True):
        """Append message."""
