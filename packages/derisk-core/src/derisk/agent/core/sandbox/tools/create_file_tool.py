...
async def execute_create_file(client: SandboxBase, conversation_id: str, description: str, path: str,
                              file_text: str) -> str:
    """
    创建新文件并写入内容。
...
async def main():
    ...
    oss_bucket_name = "antsys-antdbgpt-dev"
    from derisk_ext.sandbox.xic.xic_client import XICSandbox
    xic_client: SandboxBase = await XICSandbox.create(user_id="184089", agent="derisk", template=template, work_dir=DEFAULT_WORK_DIR,
                                                              skill_dir=skill_dir,
                                                              oss_ak=oss_ak,
                                                              oss_sk=oss_sk,
    ...
    result = await execute_create_file(xic_client,"test123","测试","/home/ubuntu/tmp/create_file_example.txt", "示例内容")
    print(result)
