...
from derisk.model.cluster.manager_base import WorkerManager
...
@register_resource(
    ...
    description=_("Default LLM client(Connect to your DERISK model serving)"),
    parameters=[
...
@register_resource(
    ...
    category=ResourceCategory.LLM_CLIENT,
    description=_("Remote LLM client(Connect to the remote DERISK model serving)"),
    parameters=[
        Parameter.build_from(
...
class RemoteLLMClient(DefaultLLMClient):
    ...
    If you start DERISK model cluster, the controller address is the address of the
    Model Controller(`derisk start controller`, the default port of model controller
    is 8000).
    Otherwise, if you already have a running DERISK server(start it by
    `derisk start webserver --port ${remote_port}`), you can use the address of the
    ...
    def __init__(
        ...
        from derisk.model.cluster import ModelRegistryClient, RemoteWorkerManager
