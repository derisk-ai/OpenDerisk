...
from .base import DERISKS_METADATA_FILE, TYPE_TO_PACKAGE
...
def create_template(
    ...
    """Create a new flow derisk"""
    if derisks_type != "flow":
    ...
    working_directory = os.path.join(working_directory, TYPE_TO_PACKAGE[derisks_type])
    ...
    if derisks_type == "flow":
        ...
        raise ValueError(f"Invalid derisks type: {derisks_type}")
...
def _create_flow_template(
    ...
    _write_derisks_toml(working_directory, name, json_dict)
...
def _create_operator_template(
    ...
    if definition_type != "python":
        raise click.ClickException(
            f"Unsupported definition type: {definition_type} for derisks type: "
            f"{derisks_type}"
        )
    # Create project structure
    _create_project_structure(working_directory, name, base_metadata.get("description"))
...
def _write_derisks_toml(working_directory: str, name: str, json_data: dict):
    """Write the derisks.toml file"""

    import tomlkit

    with open(Path(working_directory) / name / DERISKS_METADATA_FILE, "w") as f:
...
def _write_manifest_file(working_directory: str, name: str, mod_name: str):
    ...
    manifest = f"""include derisks.toml
...
from derisk.core.awel.flow import ViewMetadata, OperatorCategory, IOField, Parameter
...
from derisk.agent.util import cmp_string_equal
