...
from derisk.configs import VARIABLES_SCOPE_FLOW_PRIVATE
from derisk.core.awel.dag.base import DAG, DAGNode
from derisk.core.awel.dag.dag_manager import DAGMetadata
...
class _VariablesRequestBase(BaseModel):
    key: str = Field(
        ...
        examples=["derisk.model.openai.api_key"],
    )
...
class VariablesRequest(_VariablesRequestBase):
    ...
    For creating a new variable in the DERISK.
    """
...
class FlowPanel(BaseModel):
    ...
    source: Optional[str] = Field(
        ...
        examples=["DERISK-WEB", "DERISK-GITHUB"],
    )
...
class FlowFactory:
    ...
    def pre_load_requirements(self, flow_panel: FlowPanel):
        ...
        """
        from derisk.util.module_utils import import_from_string
