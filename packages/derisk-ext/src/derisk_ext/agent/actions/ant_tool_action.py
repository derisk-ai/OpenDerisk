...
ANT_MONITOR_MCPS: List[str] = ["mcp-antmonitor"]
ANT_MONITOR_TOOLS: List[str] = []


class AntToolAction(AntMonitorAction, BrowserAction):
    """Tool action class."""
    name = "AntTool"
    ...
    async def is_monitor_tool(self, tool_info: BaseTool, tool_pack: Optional[ToolPack]=None):
        ...
        if tool_pack and isinstance(tool_pack, MCPToolPack) and (tool_pack.name in ANT_MONITOR_MCPS):
            return True
        if tool_info.name in ANT_MONITOR_TOOLS:
            return True
        return False
    ...
    async def gen_view(self, message_id, tool_call_id,  tool_info: BaseTool, status,
        ...
        if is_monitor_tool:
            ...
            return await AntMonitorAction.gen_view(self, message_id=message_id, tool_call_id=tool_call_id,
