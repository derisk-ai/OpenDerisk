...
from derisk_client.knowledge_client import KnowledgeModel
...
class Service(BaseService):
    ...
    def create(self, request: AssetKnowledgeCreateRequest) -> AssetDocumentResponse:
        """
        Adds a public knowledge document to the search index.
        """
        knowledge_model = self.knowledge_service.get(
            {
                "knowledge_id": request.knowledge_id
            }
        )
        if not knowledge_model:
            raise ValueError(f"Knowledge space with ID {request.knowledge_id} not found.")
        tags = json.loads(str(knowledge_model.tags))
        if knowledge_model.category:
            tags.append(knowledge_model.category)
            knowledge_model.tags = tags

        if knowledge_model.storage_type == "AKG":
            combine_content = f"{knowledge_model.name}{knowledge_model.description}"[
                              :8192]
            # if knowledge_model.storage_type == "AKG":
            #     combine_content = f"{knowledge_model.knowledge_base_name}{knowledge_model.knowledge_base_desc}"[
            #                       :8192]
            #     load_documents.append({
            #         "asset_id": knowledge_model.asset_id,
            #         "asset_type": "knowledge",
            #         "knowledge_type": "document",
            #         "knowledge_base_id": knowledge_model.knowledge_base_id,
            #         "knowledge_base_name": knowledge_model.knowledge_base_name,
            #         "knowledge_base_desc": knowledge_model.knowledge_base_desc,
            #         "tags": tags,
            #         "document_name": f"{knowledge_model.knowledge_base_name}",
            #         "document_id": f"{kg_project_id.get(knowledge_model.knowledge_base_id)}",
            #         "document_content": f"",
            #         "document_desc": f"{knowledge_model.knowledge_base_desc}",
            #         "document_tags": "",
            #         "document_type": f"{knowledge_model.knowledge_type}",
            #         "create_time": f"{knowledge_model.create_time}",
            #         "update_time": f"{knowledge_model.update_time}",
            #         "document_create_at": f"{knowledge_model.create_time}",
            #         "document_update_at": f"{knowledge_model.update_time}",
            #         "word_count": 0,
            #         "document_link": f"",
            #         "creator": f"{knowledge_model.creator}",
            #         "read_count": 0,
            #         "like_count": 0,
            #         "combined_vector": embeddings.embed_query(combine_content),
            #     })
        else:
            self.add_documents(knowledge_model=KnowledgeModel(
                knowledge_base_id=knowledge_model.knowledge_id,
                knowledge_base_name=knowledge_model.name,
                knowledge_base_desc=knowledge_model.desc,
                storage_type=knowledge_model.storage_type,
                tags=tags,
                category="",
                asset_type="knowledge",
            ), index_name=request.index_name)
    ...
    def add_documents(
        ...
        """
        load_documents = []
        documents = self.knowledge_service.get_document_list(
            {
                "knowledge_id": knowledge_model.knowledge_base_id
            }
        )
        for document in documents:
            if self.check_document_exists(
                    document_id=document.doc_id,
                    index_name=index_name
            ):
                logger.warning(
                    f"document {document.doc_id},{document.doc_name} already search store exists"
                )
                continue
            chunks = self.knowledge_service.get_chunk_list(
                {
                    "doc_id": document.doc_id,
                }
            )
            document_content = "".join([chunk.content for chunk in chunks])
            combine_content = (f"{knowledge_model.knowledge_base_name}"
                               f"{knowledge_model.knowledge_base_desc}"
                               f"{document.doc_name} "
                               f"{document_content}"
                               f"{document.doc_name}"
                               f"{knowledge_model.knowledge_base_name}{knowledge_model.knowledge_base_desc}")[
                              :8192]

            document_base_dict = {
                "_id": knowledge_model.asset_id,
                "asset_id": knowledge_model.asset_id,
                "asset_type": "knowledge",
                "knowledge_type": "document",
                "knowledge_base_id": knowledge_model.knowledge_base_id,
                "knowledge_base_name": knowledge_model.knowledge_base_name,
                "knowledge_base_desc": knowledge_model.knowledge_base_desc,
                "tags": knowledge_model.tags,
                "document_name": f"{document.doc_name}",
                "text_to_embed": combine_content,
                "document_id": f"{document.doc_id}",
                "document_content": f"{document_content}",
                "document_tags": f"{document.tags}" if document.tags else "",
            }
            if document.doc_type.lower() == "yuqueurl":
                from derisk_serve.rag.models.yuque_db import KnowledgeYuqueEntity
                yuques = self.knowledge_service._yuque_dao.get_knowledge_yuque(
                ...
                if not yuques:
                    logger.error("yuque not found")
                    continue
                yuque = yuques[0]
                document_create_at = parser.parse(
                    timestr=yuque.created_at).strftime(
                    '%Y-%m-%d %H:%M:%S') if yuque and yuque.created_at else None
                document_update_at = parser.parse(timestr=yuque.updated_at).strftime(
                    '%Y-%m-%d %H:%M:%S')if yuque and yuque.updated_at else None
                document_base_dict.update({
                    "document_type": "yuque",
                    "document_create_at": f"{document_create_at}",
                    "document_update_at": f"{document_update_at}",
                    "document_desc": f"{yuque.description}" if yuque.description else "",
                    "word_count": f"{yuque.word_cnt}",
                    "document_link": f"{document.content}",
                    "creator": f"{yuque.creator_login_name}" if yuque.creator_login_name else "",
                    "read_count": f"{yuque.read_count}" if yuque.read_count else 0,
                    "like_count": f"{yuque.likes_count}" if yuque.likes_count else 0,
                    "avatar_url": f"{yuque.avatar_url}" if yuque.avatar_url else "",

                })
                load_documents.append(document_base_dict)
        search_store = self.storage_manager.create_search_store(index_name=index_name)
        print(load_documents)
        search_store.upsert(load_documents)
