...
from derisk.agent import ConversableAgent, get_agent_manager
from derisk.component import ComponentType, SystemApp
from derisk.core import ModelMetadata, ModelRequest, PromptTemplate
from derisk.core.interface.prompt import (
...
class Service(BaseService[ServeEntity, ServeRequest, ServerResponse]):
    ...
    def get_type_targets(self, prompt_type: str):
        ...
        if type == PromptType.AGENT:
            ...
            from derisk.rag.evaluation.answer import LLMEvaluationMetric
    ...
    def load_template(
        ...
        if type == PromptType.AGENT:
            ...
            cfg = Config()
            from derisk_app.scene import AppScenePromptTemplateAdapter
            ...
            if not target:
                raise ValueError("请选择一个场景用来加载模版")
            try:
                from derisk.rag.evaluation.answer import (
                    LLMEvaluationMetric,
                )
